...
export default class ViewCreator {
    ...
    /**
     * @param {import('../pages/shared/header/header-view').HeaderLink} linkItem
     * @returns {HTMLElement}
     */
    createLinkButton(linkItem) {
        const builder = new ElementBuilder('a');
        builder
            .setCssClasses([HeaderCssClasses.ITEM])
            .setTextContent(linkItem.name)
            .setClickCallback(linkItem.callback);
        return builder.getElement();
    }
